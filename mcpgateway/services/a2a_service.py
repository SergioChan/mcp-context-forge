...
class A2AAgentService:
    ...
    def _check_agent_access(
        ...
        # Team agents: check team membership (token_teams must be a list)
        # token_teams can still be None here when user_email is set but token has no teams claim
    ...
    async def invoke_agent(
        ...
        # One failing request does not abort others. If all fail, we raise after rollback.
        # On DB/transaction failure we rollback and re-raise.
        # ═══════════════════════════════════════════════════════════════════════════
        ...
        try:
            ...
            # Lock all rows in id order to avoid deadlocks (may have fewer than unique_names if some missing)
            ...
            # One entry per request: either a payload dict (has "agent_id") or an error dict ("error", "code").
            # Auth application (URL query params + headers) is done in Rust; we only decrypt and pass through.
            ...
            for req in requests:
                ...
                # SECURITY: Check visibility/team access WHILE ROW IS LOCKED (404 not 403)
                if not self._check_agent_access(agent, user_email, token_teams):
                    batch_payloads.append({"error": f"A2A Agent not found with name: {agent_name}", "code": "not_found", "agent_name": agent_name})
                    continue
                ...
                if agent_auth_type == "query_param" and agent_auth_query_params:
                    ...
                    # URL with query params is applied in Rust; we only pass decrypted params
            ...
            # If every request failed validation, raise so caller gets a single error
            ...
            # CRITICAL: Commit and release connection before HTTP calls; do NOT close the session
            # — the session may be request-scoped (e.g. FastAPI Depends(get_db)); closing it here
            # would close the same session reference in the caller and break any later use.
            db.commit()
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        # PHASE 2: Batch HTTP via Rust invoker (1..N). Same result shape as Rust for
        # easy parsing: each slot is either {status_code, body, parsed} or {error, code, status_code}.
        # ═══════════════════════════════════════════════════════════════════════════
        ...
        for idx, p in enumerate(batch_payloads):
            ...
            # Rust applies auth (query params + headers); no Python fallback.
        ...
        # Rust returns list of (id, A2AResponsePy, duration_secs)
        ...
        # PHASE 3: Record metrics and last_interaction in one place (buffer + DB).
        try:
