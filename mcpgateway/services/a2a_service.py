...
class A2AAgentService:
    ...
    async def invoke_agents_batch(
        ...
        *,
        ...
        """Invoke multiple A2A agents concurrently via Rust invoke_batch.

        Each item in `requests` is (agent_name, parameters). Runs the same
        DB/auth/visibility logic as invoke_agent per agent, then performs all
        HTTP calls concurrently via rust_a2a.invoke_batch. Returns a list of
        response dicts in the same order as `requests` (same shape as
        invoke_agent return, or {"error": "..."} on failure).

        Args:
            db: Database session (commit() is called after each agent lookup; session is not closed).
            requests: List of (agent_name, parameters) to invoke.
            interaction_type: Type of interaction (default "query").
            user_id: Identifier of the user initiating the call.
            user_email: Email of the user initiating the call.
            token_teams: Teams from JWT token.
            timeout_secs: Per-request HTTP timeout in seconds; uses settings.httpx_read_timeout if None.

        Returns:
            List of response dicts, one per request, in order.
        """
        if not requests:
            return []
        ...
        timeout = timeout_secs if timeout_secs is not None else settings.httpx_read_timeout
        correlation_id = get_correlation_id()

        # Prepare each agent (same DB/auth/visibility logic as invoke_agent); commit after each to release row lock
        batch_requests: List[Tuple[str, Dict[str, Any], Dict[str, str]]] = []
        metadata_list: List[Dict[str, Any]] = []

        for agent_name, parameters in requests:
            prep = self._prepare_agent_invocation(db, agent_name, parameters, interaction_type, user_email, token_teams)
            db.commit()
            call_start_time = datetime.now(timezone.utc)
            batch_requests.append((prep["url"], prep["request_data"], prep["headers"]))
            metadata_list.append(
                {
                    ...
                    "auth_query_params_decrypted": prep["auth_query_params_decrypted"],
                    "call_start_time": call_start_time,
                    "interaction_type": prep["interaction_type"],
                    "protocol_version": prep["protocol_version"],
                }
            )

        # Perform all HTTP calls concurrently via Rust invoke_batch (no DB connection held)
        try:
            from mcpgateway_rust.services import a2a_service as rust_a2a  # pylint: disable=import-outside-toplevel

            rust_requests = [(url, data, hdrs) for url, data, hdrs in batch_requests]
            results = await rust_a2a.invoke_batch(rust_requests, timeout)
        except (ImportError, AttributeError):
            # Fallback: run single invokes sequentially when Rust batch is not available (each uses fresh session)
            out = []
            for agent_name, parameters in requests:
                try:
                    with fresh_db_session() as fallback_db:
                        resp = await self.invoke_agent(fallback_db, agent_name, parameters, interaction_type, user_id=user_id, user_email=user_email, token_teams=token_teams)
                    out.append(resp)
                except (A2AAgentNotFoundError, A2AAgentError) as e:
                    out.append({"error": str(e)})
            return out

        # Map batch results to response dicts; record metrics and last_interaction per agent
        out: List[Dict[str, Any]] = []
        for result, meta in zip(results, metadata_list):
            agent_name = meta["agent_name"]
            agent_id = meta["agent_id"]
            auth_query_params_decrypted = meta["auth_query_params_decrypted"]
            call_start_time = meta["call_start_time"]
            interaction_type_meta = meta["interaction_type"]
            success = False
            error_message = None
            response = None
            ...
            if status_code == 0:
                error_message = sanitize_exception_message(body or "Transport error", auth_query_params_decrypted)
                structured_logger.log(
                    level="ERROR",
                    message=f"A2A batch call failed: {agent_name}",
                    component="a2a_service",
                    user_id=user_id,
                    user_email=user_email,
                    correlation_id=correlation_id,
                    metadata={"event": "a2a_batch_call_failed", "agent_name": agent_name, "agent_id": agent_id, "transport_error": True},
                )
                out.append({"error": error_message})
            elif 200 <= status_code < 300 and parsed is not None:
                ...
                try:
                    ...
                    out.append({"error": sanitize_exception_message(error_message, auth_query_params_decrypted)})

            call_duration_ms = (datetime.now(timezone.utc) - call_start_time).total_seconds() * 1000
            if success:
                structured_logger.log(
                    level="INFO",
                    message=f"A2A batch call completed: {agent_name}",
                    component="a2a_service",
                    user_id=user_id,
                    user_email=user_email,
                    correlation_id=correlation_id,
                    duration_ms=call_duration_ms,
                    metadata={"event": "a2a_batch_call_completed", "agent_name": agent_name, "agent_id": agent_id, "status_code": status_code},
                )
            try:
                from mcpgateway.services.metrics_buffer_service import get_metrics_buffer_service  # pylint: disable=import-outside-toplevel
                ...
                metrics_buffer.record_a2a_agent_metric_with_duration(
                    a2a_agent_id=agent_id,
                    response_time=call_duration_ms / 1000.0,
                    success=success,
                    interaction_type=interaction_type_meta,
                    error_message=error_message,
                )
            except Exception as metrics_error:
                logger.warning("Failed to record A2A metrics for '%s': %s", agent_name, metrics_error)
            # Update last interaction timestamp (quick separate write, reacquire lock and re-check enabled)
            try:
                with fresh_db_session() as ts_db:
                    db_agent = get_for_update(ts_db, DbA2AAgent, agent_id)
                    if db_agent and getattr(db_agent, "enabled", False):
