...
export const serverSideToolSearch = async function (searchTerm) {
  ...
  container.querySelectorAll('input[name="associatedTools"]').forEach((cb) => {
  ...
  if (searchTerm.trim() === "") {
    ...
    try {
      ...
      const viewPublicCb = document.getElementById("add-server-view-public");
  ...
  try {
    ...
    // Respect View Public checkbox state: include team_id only when unchecked
    const editViewPublicCb = document.getElementById("add-server-view-public");
    if (selectedTeamId && (!editViewPublicCb || !editViewPublicCb.checked)) {
...
export const serverSidePromptSearch = async function (searchTerm) {
  ...
  try {
    ...
    // Respect View Public checkbox state: include team_id only when unchecked
    const addViewPublicCb = document.getElementById("add-server-view-public");
...
`;
  ...
  if (searchTerm.trim() === "") {
    ...
    try {
      ...
      // Respect View Public checkbox state: include team_id only when unchecked
      const viewPublicCb = document.getElementById("edit-server-view-public");
      const urlTeamId = new URL(window.location.href).searchParams.get(
        "team_id"
      );
      if (urlTeamId && (!viewPublicCb || !viewPublicCb.checked)) {
        ...
        try {
          ...
          if (dataAttr) {
            ...
            if (Array.isArray(serverTools)) {
              serverTools.forEach((t) => serverToolNames.add(String(t)));
            }
          }
          ...
          if (
            ...
            currentCheckedTools.forEach((t) => persistedToolIds.add(String(t)));
          }
          ...
          if (persistedToolIds.size > 0 || serverToolNames.size > 0) {
            ...
            checkboxes.forEach((cb) => {
              ...
              const toolName =
                ...
                (window.Admin.toolMapping &&
                  window.Admin.toolMapping[cb.value]);
              if (
  ...
  try {
    ...
    // Respect View Public checkbox state: include team_id only when unchecked
    const editViewPublicCb = document.getElementById("edit-server-view-public");
    if (selectedTeamId && (!editViewPublicCb || !editViewPublicCb.checked)) {
      params.set("team_id", selectedTeamId);
    }
    const searchUrl = `${window.ROOT_PATH}/admin/tools/search?${params.toString()}`;
  ...
  if (searchTerm.trim() === "") {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          const persistedPromptIds = getEditSelections("edit-server-prompts");
          ...
          if (dataAttr) {
            ...
            if (Array.isArray(serverPrompts)) {
              serverPrompts.forEach((p) => serverPromptIds.add(String(p)));
            }
          }
  ...
  if (searchTerm.trim() === "") {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          const persistedResIds = getEditSelections("edit-server-resources");
          ...
          if (dataAttr) {
            ...
            if (Array.isArray(serverResources)) {
              serverResources.forEach((r) => serverResIds.add(String(r)));
            }
          }
          ...
          if (Array.isArray(serverResources) && serverResources.length > 0) {
            ...
            checkboxes.forEach((cb) => {
              ...
              const resourceName =
                ...
                (window.Admin.resourceMapping &&
                  window.Admin.resourceMapping[cb.value]);
