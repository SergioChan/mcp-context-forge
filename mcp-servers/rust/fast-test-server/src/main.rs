...
impl FastTestServer {
    ...
    #[tool(description = "Echo back the provided message. Useful for testing connectivity and latency. Optionally delay the response by a specified number of milliseconds. If delay_stddev is provided (and > 0), the actual delay is sampled from a normal distribution with mean=delay and the given standard deviation, clamped to >= 0.")]
    async fn echo(
        &self,
